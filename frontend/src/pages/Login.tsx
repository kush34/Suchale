...
export const Input = ({
  ...
  <div
    className={`relative flex items-center h-10 w-full rounded-lg bg-black/30 px-3 py-2 text-sm transition-colors focus-within:ring-1 focus-within:ring-blue-400 focus-within:border-blue-400 ${className}`}
  >
...
export const Button = ({
  ...
  switch (variant) {
    case "default":
      variantClasses =
        "bg-white/15 text-white shadow-xl hover:bg-white/25 active:scale-[0.99] transition-transform duration-100 ease-in-out";
      break;
    case "ghost":
      variantClasses =
        "bg-transparent border border-gray-700 text-white hover:bg-white/10 active:scale-[0.99] transition-transform duration-100 ease-in-out";
      break;
    case "link":
      variantClasses =
...
const Login = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      const { googleSignInPopUp } = await import("@/config/firebaseConfig");
  ...
  return (
    <div
      className="min-h-screen flex items-center justify-center p-4 bg-gray-950 font-sans"
      style={{
        backgroundImage:
          "radial-gradient(circle at 50% 120%, rgba(50, 80, 200, 0.45) 0%, rgba(10, 10, 10, 1) 50%)",
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-400">
            Log in to your account and continue managing your projects, ideas,
            and progress.
          </p>
        </div>
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          {flag1 && (
            <ErrorMessage message="Please enter both your email address and password." />
          )}
          {flag2 && (
            <ErrorMessage message="Login failed: Your account has been blocked or disabled." />
          )}
          {flag3 && (
            <ErrorMessage message="An unexpected error occurred during login. Please try again." />
          )}
